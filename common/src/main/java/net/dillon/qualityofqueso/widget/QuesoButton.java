...
public class QuesoButton extends Button {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (!this.isMouseOver(mouseX, mouseY) || !this.active || !clientOptionsInstance().getGeneralOptions().tooltips.enabled()) {
            return;
        }
     ...
     */
    protected final void renderBaseTexture(GuiGraphics graphics) {
     ...
     */
    protected final void renderHoveredTexture(GuiGraphics graphics, double mouseX, double mouseY) {
        if (!this.canBeActive.get() || !this.isMouseOver(mouseX, mouseY)) {
            return;
        }

        int xy = getTransferButtonXY(this);
        graphics.blit(ofQoQ("textures/gui/sprites/button/" + BASE_BUTTON_HOVERED_PATH + ".png"), this.getX() - 1, this.getY() - 1, 0.0F, 0.0F, xy, xy, xy, xy);
    }

    /**
     * Renders the button texture.
     */
    protected final void renderButtonTexture(String id, AbstractWidget buttonReference, GuiGraphics graphics, double mouseX, double mouseY) {
        int xy = getTransferButtonXY(this);
        this.renderBaseTexture(graphics);
        graphics.blit(ofQoQ("textures/gui/sprites/button/" + id + this.getAppendedTexture()), buttonReference.getX() - 1, buttonReference.getY() - 1, 0.0F, 0.0F, xy, xy, xy, xy);
     ...
     */
    protected final void extractMoveAmountTooltip(GuiGraphics graphics, int mouseX, int mouseY) {
