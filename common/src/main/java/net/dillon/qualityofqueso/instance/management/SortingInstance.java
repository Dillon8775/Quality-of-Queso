...
public class SortingInstance extends ManagementInstance {
    ...
    private Map<Item, Integer> getCreativeSearchTabOrder() {
        ...
        if (client.getConnection() != null) {
            var lookupProvider = client.level != null ? client.level.registryAccess() : client.getConnection().registryAccess();
            boolean hasOpPerms = client.player != null && client.player.canUseGameMasterBlocks();

            CreativeModeTabs.tryRebuildTabContents(client.getConnection().enabledFeatures(), hasOpPerms, lookupProvider);
        }
    ...
    public void sortItems() {
        ...
        if (inventoryScreen) {
            ...
            int end = options().management.includeHotbar ? 45 : 36;
    ...
    private int getSortableContainerSize() {
        ...
        // Most container menus append the player's 36 slots to the end.
        // For modded screens with no cached container, infer top-container slots from that layout.
        if (totalSlots > 36) {
    ...
    private List<ItemStack> normalizeStacks(List<ItemStack> input) {
        ...
        for (ItemStack stack : input) {
            ...
            for (StackGroup group : groups) {
                if (ItemStack.isSameItemSameTags(group.sample, stack)) {
                    found = group;
                    break;
                }
            }
