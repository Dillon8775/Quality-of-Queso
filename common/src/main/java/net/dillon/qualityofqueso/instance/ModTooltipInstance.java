...
public class ModTooltipInstance extends ManagementInstance {
    ...
    public void displaySingleMovingTooltips(GuiGraphicsExtractor graphics, Font font, int mouseX, int mouseY, CallbackInfo ci) {
        ...
        boolean hasKeyDown = quickDropHovered ? hasDropOnlyOneItemModifierDown() : hasAllQuickDropModifiersDown() || canScrollMoveAndHasScrollModifierDown();
        boolean droppingOne = hoveredSlotHasItem && hasDropOnlyOneItemModifierDown();
        ...
        if (!isCreativeInventoryScreen(instance().getScreen()) && bl && !onlySingleMoveModifierDown && (hoveredSlot == null || hoveredSlot.getItem().count() > 1)) {
            ...
            if (hasAllQuickDropModifiersDown() && hoveredSlotHasItem(hoveredSlot)) {
                ...
                if (containerScreen && !shouldButtonBeActive(false, null)) {
                    ...
                    if (hasDropOnlyOneItemModifierDown()) { // Display that item name w/ the move amount
