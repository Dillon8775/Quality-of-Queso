...
public class ExtractingInstance extends ManagementInstance {
    ...
    public void extractButtons(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        if (validScreen || dropperDispenserOrHopperScreen) {
            ...
            if (!inventoryScreen
                    && (ContainerHelper.isTrackedFilteringActive() || !options().buttonDisplayOptions.displayFiltering.filteredContainersOnly()
                    && ((containerScreen || dropperDispenserOrHopperScreen) && options().management.transferring.buttonOrKeyOrKeyOnly()))) {
