...
public class TransferInstance extends ManagementInstance {
    ...
    public boolean tryMoveSingleFromHovered(Slot hoveredSlot, int bl) {
        ...
        if (hoveredIndex >= fromStart
            ...
            if (tryMoveIntoPreferredSlot(hoveredSlot, toContainer, targetItem, moveAmount)) {
                return true;
            }
        ...
        for (int i = fromStart; i < fromEnd; i++) {
            ...
            if (tryMoveIntoPreferredSlot(sourceSlot, toContainer, targetItem, moveAmount)) {
                return true;
            }
     ...
     */
    private boolean tryMoveIntoPreferredSlot(Slot sourceSlot, boolean toContainer, ItemStack targetItem, int amount) {
        if (!toContainer) {
            return false;
        }
        ...
        if (instance().getScreen() instanceof AbstractFurnaceScreen<?> && AbstractFurnaceBlockEntity.isFuel(targetItem)) {
    ...
    private boolean moveSingleCraftingResult(Slot resultSlot) {
        ...
        for (int i = playerStart; i < playerEnd; i++) {
            ...
            if (!ItemStack.isSameItemSameTags(slot.getItem(), carried)) {
                continue;
            }
