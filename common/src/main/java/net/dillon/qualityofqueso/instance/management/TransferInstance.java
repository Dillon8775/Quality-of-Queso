...
public class TransferInstance extends ManagementInstance {
        ...
        try {
            ...
            if (parkedCursorSlot != -1) {
                ...
                if (!instance().getScreenMenu().getCarried().isEmpty()) {
                    ...
                    performClickSlot(instance().getScreen(), freshSourceSlot, sourceSlot.index, 0, ClickType.PICKUP);
                }
    ...
    public void swapSlots(int a, int b, List<Integer> sortableSlots) {
        ...
        // Bundle interactions can consume carried items, so route these swaps
        // through an empty temp slot to force a true swap.
        if (isBundle(aStack) || isBundle(bStack)) {
            int temp = findTemporaryEmptySlotForBundle(a, b, sortableSlots);
            if (temp != -1) {
                clickSlot(a);
                clickSlot(temp);
                clickSlot(b);
