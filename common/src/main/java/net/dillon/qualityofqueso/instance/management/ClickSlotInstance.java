...
public class ClickSlotInstance extends ManagementInstance {
    ...
    private static @Nullable Recipe<?> selectedBulkCraftRecipe;

    private record ActiveTradeAllTask(int containerId, int selectedTradeIndex, int remainingClicks, int emptyResultTicks) {}
    private record ActiveBulkCraftTask(int containerId, @Nullable Recipe<?> recipe, int remainingClicks, int emptyResultTicks) {}

    public ClickSlotInstance(QuesoScreen screen) {
        super(screen);
    }

    public static void setSelectedBulkCraftRecipe(@Nullable Recipe<?> recipe) {
    ...
    public void bulkCraftForSelectedRecipe(int slotId, ClickType containerInput, CallbackInfo ci) {
        ...
        Recipe<?> recipe = ((RecipeBookComponentAccessor) getRecipeBookComponent(instance().getScreen()))
                .getGhostRecipe()
                .getRecipe();
