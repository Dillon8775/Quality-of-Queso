...
public class ClientMain {
    ...
    private static void checkClientConfigs() {
        ...
        if (ModClientOptions.INSTANCE.getInstance() == null) {
            ModConstants.LOGGER.error("Quality of Queso's client-config is null! Please delete it and relaunch your game.");
            shouldStop = true;
            configName = ModConstants.DEFAULT_CLIENT_CONFIG_FILE_NAME;
        }
        if (ContainerData.INSTANCE.getInstance() == null) {
            ModConstants.LOGGER.error("Quality of Queso's tracked containers config is null! Please delete it and relaunch your game.");
            shouldStop = true;
            configName = ModConstants.DEFAULT_CONTAINER_DATA_FILE_NAME;
        }
        if (LockedContainerSlots.INSTANCE.getInstance() == null) {
            ModConstants.LOGGER.error("Quality of Queso's locked container slots config is null! Please delete it and relaunch your game.");
            shouldStop = true;
            configName = ModConstants.DEFAULT_LOCKED_CONTAINER_SLOTS_FILE_NAME;
        }
        if (LockedPlayerSlots.INSTANCE.getInstance() == null) {
            ModConstants.LOGGER.error("Quality of Queso's locked player slots config is null! Please delete it and relaunch your game.");
            shouldStop = true;
