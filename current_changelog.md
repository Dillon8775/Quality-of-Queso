...
- You can disable the armor slot outlights displaying by disabling the "Armor Slot Outlines" option under the new "Accessibility..." options.
- Added smart positioning for armor status, depending on if an item is being displayed near the hotbar.
...
- This change was made because it is more useful to select certain slots rather than exclude them, and it's harder to do this when having to use ALT + SHIFT, so now you just have to hold ALT to select slots :).
- Using the drag sorting feature now hides tooltips from being rendered.
...
- "Elytra Alarm Sound Delay", the delay for playing the elytra alarm sound.
- "Use Old Search Bar Texture", which uses the old search bar texture.
- "Armor Slot Outlines", which disables armor slot outlines with the armor status feature.
- "Auto-Focus into Recipe Book", which prioritizes auto-focusing into the recipe book when "Inventory Searching" is enabled. Disable this option if you want to prioritize auto-focusing into the inventory search bar.
...
- "Helpful Tooltips" option is now more effective.
- Changed "on/off" and "yes/no" text (upon other option) colors to be orange/gray, giving a more QoQ-vibe.
- The "Quick Drop" button now appears if you are using the shortcut key, regardless of what you have the Quick Drop button option set to.
